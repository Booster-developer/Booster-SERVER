...
exports.readProgressList = async (req, res)=> {
    const query = `SELECT orders.order_idx, store_name order_store_name, engine_time order_time, file_name order_title, file_extension order_extension, order_state, COUNT(*) count FROM ((Booster.ORDER orders JOIN Booster.FILE USING(order_idx)) JOIN Booster.STORE USING(store_idx)) JOIN Booster.ENGINE engine ON engine.user_idx = orders.user_idx WHERE engine.user_idx = ${req.user_idx} AND orders.order_state BETWEEN 1 AND 3 GROUP BY order_idx`;
