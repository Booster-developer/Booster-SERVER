...
exports.readProgressDetailList = async (req, res)=> {
    const query = `SELECT orders.order_idx, store_name order_store_name, order_state, engine_time order_time, order_comment, file_name, file_extension, file_price, file_color, file_range_start, file_range_end, file_sided_type, file_direction, file_collect, file_copy_number FROM ((Booster.ORDER orders JOIN Booster.FILE USING(order_idx)) JOIN Booster.STORE USING(store_idx)) JOIN Booster.ENGINE engine USING(user_idx) WHERE engine.user_idx = ${req.user_idx} AND orders.order_idx = ${req.params.order_idx}`;
